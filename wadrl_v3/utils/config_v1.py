...
def precompute_drone_time_energy(
    ...
    payloads: List[float],               # payload carried into j (common for pickup)
    ...
    Returns:
      ...
      E_cons[d][i][j]   : Joules
    Using payloads[j] as the weight transported to j.
    Optionally marks i->j infeasible (inf) if i->j plus j->0 exceeds drones.max_trip_time_s.    def _apply_drone_move(self, d: int, j: int):
...
if __name__ == "__main__":
    cfg, meta = build_config_from_files(
        ...
        customers_txt_path="/home/saphyiera/HUST/WADRL_PVRP/data/random_data/10.10.2.txt")
