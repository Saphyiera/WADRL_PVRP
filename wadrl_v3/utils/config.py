...
class Trucks:
    ...
    slot_coeffs: List[float]          # length-L per-hour speed multipliers, e.g., len=12
    slot_len_hours: float = 1.0       # slot length (hours)

    @staticmethod
    def from_json_dict(num_trucks: int, j: Dict) -> "Trucks":
        vmax = float(j["V_max (m/s)"])
        # T (hour) keys like "0-1", "1-2", ...
        slot_map = j["T (hour)"]
        # Sort by the starting hour index to get a consistent array
    ...
    def speed_at(self, t_seconds: float) -> float:
        """
        Time-varying truck speed v_trk(t) = vmax * sigma_a for t in slot a.
        Slots repeat every L * slot_len (cyclic day profile).
        """
        cycle = self.L * self.slot_len_seconds
        t = t_seconds % cycle
        slot = int(t // self.slot_len_seconds)
        slot = max(0, min(self.L - 1, slot))
        return self.vmax * self.slot_coeffs[slot]

    def calc_truck_travel_time(self, start_time_s: float, distance_m: float) -> float:
        """
        Integrate distance over piecewise-constant speed slots starting at start_time_s.
        Returns the travel time (seconds) needed to traverse 'distance_m'.
        """
        if distance_m <= 0:
            return 0.0
        rem = distance_m
        ...
        while rem > 1e-9:
            ...
            if can_travel >= rem:
                ...
                rem -= can_travel
                total_dt += dt_slot
                t += dt_slot  # move to next slot boundary
...
class Drone:
    ...
    def flight_time_s(self, dist_m: float) -> float:
        """
        Time to go depot/any node at same altitude: takeoff + cruise + landing.
        We treat vertical segments as: t_to = cruise_alt / takeoff_speed, t_ld = cruise_alt / landing_speed.
        """
        if dist_m < 0:
    ...
    def energy_j(self, dist_m: float, payload_kg: float) -> float:
        """
        Simple linear power model: P(w)=beta*w + gama (Watts).
        Energy E = P * time. We apply it over the whole flight_time_s.
        """
...
class Drones:
    ...
    ids: List[str] = field(default_factory=list)  # keep original keys if you parse from a dict
    max_trip_time_s: Optional[float] = None  # NEW: scenario cap (optional)
    ...
    def head(self, m: int) -> "Drones":
        "Return a shallow 'subset' fleet of the first m drones."
...
class Customer:
    ...
    y: float
    demand: float                  # kg
    only_truck: bool               # 1 if truck-only; 0 otherwise
...
class Customers:
    ...
    def from_csv(
        ...
        with open(path, "r", encoding="utf-8") as f:
            sample = f.read(4096)
            f.seek(0)
            dialect = csv.Sniffer().sniff(sample) if delimiter is None else csv.get_dialect("excel")
            reader = csv.reader(f, delimiter=(delimiter or dialect.delimiter))
            if has_header:
                next(reader, None)
            for row in reader:
...
def precompute_drone_time_energy(
    drones: Drones,
    coords: List[Tuple[float, float]],   # index 0 = depot, 1..N = customers
    payloads: List[float],               # 0..N (payload carried on leg i->j; for i!=0 you decide your convention)
) -> Tuple[List[List[List[float]]], List[List[List[float]]]]:
    """
    Returns:
      T_drone[d][i][j]  : seconds
      E_cons[d][i][j]   : Joules
    Using payloads[j] as the weight being carried into j (common for pickup).
    """
    D = drones.D
    V = len(coords)
    T_dr = [[[0.0 for _ in range(V)] for _ in range(V)] for _ in range(D)]
    E_dr = [[[0.0 for _ in range(V)] for _ in range(V)] for _ in range(D)]
    ...
    for dd, drone in enumerate(drones.drones):
        for i in range(V):
            for j in range(V):
                ...
                t = drone.flight_time_s(dist)
                # For pickup, energy depends on payload being transported to j
                w = max(0.0, payloads[j] if j < len(payloads) else 0.0)
                e = drone.energy_j(dist, w)
                T_dr[dd][i][j] = t
                E_dr[dd][i][j] = e
    return T_dr, E_dr

# --- NEW: scenario metadata for counts/limits parsed from the .txt preamble ---
...
# --- NEW: robust TXT parser that skips preamble and reads the table ---

def load_customers_with_meta_from_txt(path: str | Path) -> Tuple[Customers, ScenarioMeta]:
    """
    Expects a preamble like:
      number_staff 2
      number_drone 2
      droneLimitationFightTime(s) 3600
      Customers 20
    followed by a header line and N data lines (whitespace/tab separated).
    """
    preamble = {}
    rows = []
    header_seen = False
    with open(path, "r", encoding="utf-8") as f:
        for raw in f:
            ...
            if not line:
                continue
            # detect header row by its column names (first token 'Coordinate' works well)
            if not header_seen and line.lower().startswith("coordinate"):
                header_seen = True
                continue
            if not header_seen:
                # parse "key value" lines (two tokens min)
                parts = line.split()
                if len(parts) >= 2:
                    key = parts[0]
                    val = parts[1]
            ...
            parts = line.split()
            if len(parts) < 6:
                # tolerate occasional double-space mis-splits; try to be strict:
    ...
    if not rows:
        raise ValueError("No customer records parsed from the TXT.")

    customers = Customers.from_list(rows)

    # Build meta (with safe defaults if a key is missing)
    meta = ScenarioMeta(
