...
class PVRPDEnv:
    ...
    def __init__(
        ...
        *,
        ...
        # max episode length
        max_steps: int | None = 512, timeout_penalty: float = -1e7,
        show_moves: bool = False,
        unserved_penalty = 1e6
    ):
        """
        Args:
        ...
        coords_arr = np.array(self.coords, dtype=np.float32)  # (V,2)
        depot_xy = coords_arr[0]
        rel = coords_arr - depot_xy
        max_radius = float(np.linalg.norm(rel, axis=1).max())
        if max_radius < 1e-6:
            max_radius = 1.0
        self._coord_shift = torch.tensor(depot_xy, dtype=torch.float32, device=self.device)
        self._coord_scale = torch.tensor(1.0 / max_radius, dtype=torch.float32, device=self.device)

        # demands: divide by max demand
        demands_arr = np.asarray(self.demands, dtype=np.float32)
        max_demand = float(demands_arr.max())
        if max_demand <= 0:
            max_demand = 1.0
        self._demand_scale = torch.tensor(1.0 / max_demand, dtype=torch.float32, device=self.device)

        # service times: divide by global max (truck/drone)
        svc_arr = np.stack([self.svc_trk, self.svc_dr], axis=-1)  # (V,2)
        max_svc = float(np.max(svc_arr))
        if max_svc <= 0:
            max_svc = 1.0
        self._svc_scale = torch.tensor(1.0 / max_svc, dtype=torch.float32, device=self.device)

        # drones: speeds, capacities, energy
        takeoff_speeds = [d.takeoff_speed for d in cfg.drones.drones]
        cruise_speeds  = [d.cruise_speed  for d in cfg.drones.drones]
        landing_speeds = [d.landing_speed for d in cfg.drones.drones]
        speed_max = float(max(takeoff_speeds + cruise_speeds + landing_speeds))
        if speed_max <= 0:
            speed_max = 1.0
        self._dr_speed_scale = torch.tensor(1.0 / speed_max, dtype=torch.float32, device=self.device)

        cap_max = float(max(d.capacity for d in cfg.drones.drones))
        if cap_max <= 0:
            cap_max = 1.0
        self._dr_cap_scale = torch.tensor(1.0 / cap_max, dtype=torch.float32, device=self.device)

        power_max = float(max(d.battery_power for d in cfg.drones.drones))
        if power_max <= 0:
            power_max = 1.0
        self._dr_energy_scale = torch.tensor(1.0 / power_max, dtype=torch.float32, device=self.device)

        # trips and intra-trip time (for features, not constraints)
        if self.max_trips_per_drone is not None:
            self._dr_trip_scale = torch.tensor(1.0 / float(self.max_trips_per_drone),
                                               dtype=torch.float32, device=self.device)
        else:
            # heuristic: treat "no limit" as ~10 trips scale
            self._dr_trip_scale = torch.tensor(0.01, dtype=torch.float32, device=self.device)

        if self.trip_time_cap is not None:
            self._trip_time_scale = torch.tensor(1.0 / self.trip_time_cap,
                                                 dtype=torch.float32, device=self.device)
        else:
            # fall back to a coarse scale; you can tune this
            self._trip_time_scale = torch.tensor(1e-3, dtype=torch.float32, device=self.device)
    ...
    def sample_weights(self):
        ...
        # return self.w
        w1 = float(np.random.choice(np.linspace(0.0, 1.0, 11)))
        w2 = 1.0 - w1
        self.w = torch.tensor([w1, w2], dtype=torch.float32, device=self.device)
        return self.w
    ...
    def reset(self) -> Dict[str, torch.Tensor]:
        ...
        F1_0, F2_0 = self._compute_F1_F2(include_unserved_penalty=False)
        self._prev_F1 = float(F1_0)
        self._prev_F2n = float(F2_0 / max(1, self.N))
    ...
    def _reset_weights(self):
        if self.weight_mode == "fixed":
            ...
            w1 = float(np.random.choice(np.linspace(0.0, 1.0, 5)))
            w2 = 1.0 - w1
            self.w = torch.tensor([w1, w2], dtype=torch.float32, device=self.device)
    ...
    def _build_obs(self) -> Dict[str, torch.Tensor]:
        ...
        coords = torch.tensor(self.coords, dtype=torch.float32, device=self.device)  # (V,2)
        coords = (coords - self._coord_shift) * self._coord_scale
        fx[:, 0:2] = coords

        fx[:, 2] = torch.tensor(self.only_truck, dtype=torch.float32, device=self.device)

        demands = torch.tensor(self.demands, dtype=torch.float32, device=self.device)
        fx[:, 3] = demands * self._demand_scale

        svc_trk = torch.tensor(self.svc_trk, dtype=torch.float32, device=self.device)
        svc_dr  = torch.tensor(self.svc_dr,  dtype=torch.float32, device=self.device)
        fx[:, 4] = svc_trk * self._svc_scale
        fx[:, 5] = svc_dr  * self._svc_scale
        ...
        truck_coords = torch.tensor([self.coords[i] for i in self.pos_trk.tolist()],
                                    dtype=torch.float32, device=self.device)  # (K,2)
        truck_coords = (truck_coords - self._coord_shift) * self._coord_scale
        t_feats[:, 0:2] = truck_coords
        ...
        drone_coords = torch.tensor([self.coords[i] for i in self.pos_dr.tolist()],
                                    dtype=torch.float32, device=self.device)  # (D,2)
        drone_coords = (drone_coords - self._coord_shift) * self._coord_scale
        d_feats[:, 0:2] = drone_coords
        ...
        for dd, drone in enumerate(self.cfg.drones.drones):
            d_feats[dd, 2] = drone.takeoff_speed * self._dr_speed_scale
            d_feats[dd, 3] = drone.cruise_speed  * self._dr_speed_scale
            d_feats[dd, 4] = drone.landing_speed * self._dr_speed_scale
            d_feats[dd, 5] = drone.capacity      * self._dr_cap_scale
            d_feats[dd, 6] = drone.battery_power * self._dr_energy_scale

        d_feats[:, 7]  = self.Ecur * self._dr_energy_scale
        d_feats[:, 8]  = self.trip_idx.float() * self._dr_trip_scale
        d_feats[:, 9]  = (~self.dr_done).float()
        d_feats[:, 10] = self.trip_elapsed * self._trip_time_scale
        d_feats[:, 11] = trips_left * self._dr_trip_scale
        drones_ctx = d_feats.unsqueeze(0)  # (1,D,12)
    ...
    def _mask_truck(self):
        ...
        allow_depot = bool(self.served[1:].all().item())
        mask[:, :, 0] = allow_depot
    ...
    def _compute_F1_F2(self, include_unserved_penalty=True) -> Tuple[float, float]:
        ...
        if num_sunsev > 0 and include_unserved_penalty:
            F2 += num_sunsev * self.unserved_penalty
        return F1 / 3600.0, F2 / 3600.0
    ...
    def _compute_step_reward(self, done: bool) -> float:
        ...
        F1, F2 = self._compute_F1_F2(include_unserved_penalty=done)
        F2n = F2 / max(1, self.N)  # normalize F2 by N to stabilize scale
        # F1, F2 = F1 / 3600, F2 / 3600
        # Shaped, telescoping reward on the DELTA of the weighted objective.
        w1, w2 = float(self.w[0].item()), float(self.w[1].item())
        cur = max(w1 * F1, w2 * F2n)
        prev = max(w1 * self._prev_F1, w2 * self._prev_F2n)
        r = -(cur - prev)
