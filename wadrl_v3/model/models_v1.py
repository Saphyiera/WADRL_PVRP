...
class WADRLPolicy(nn.Module):
    ...
    def __init__(
        ...
        self.mem_dim = d_model
        self.mem_window = int(mem_window)
        ...
        # FiLM conditioning from (w1,w2)
        self.film_graph  = nn.Linear(2, 2 * d_model)
        self.film_truck  = nn.Linear(2, 2 * d_model)
        self.film_drone  = nn.Linear(2, 2 * d_model)
        ...
        self.ctx_mix     = SelfEncoder(d_model, nhead, depth_c)
        ...
        # memory (transformer over pooled context)
        if self.use_mem:
            self.mem_in = nn.Sequential(
                nn.Linear(4 * d_model, d_model), nn.GELU()
            )
            self.mem_pos = nn.Parameter(torch.zeros(self.mem_window + 2, d_model))
            ...
            self.mem_cell = nn.TransformerEncoder(mem_layer, num_layers=mem_depth)
    ...
    def forward(
        ...
        # FiLM modulation
        g_g, b_g = self.film_graph(w_pair).chunk(2, dim=-1)
        g_t, b_t = self.film_truck(w_pair).chunk(2, dim=-1)
        g_r, b_r = self.film_drone(w_pair).chunk(2, dim=-1)
        H = H * (1.0 + g_g.unsqueeze(1)) + b_g.unsqueeze(1)
        T = T * (1.0 + g_t.unsqueeze(1)) + b_t.unsqueeze(1)
        R = R * (1.0 + g_r.unsqueeze(1)) + b_r.unsqueeze(1)

        # --- 2) encode nodes & memory update ---
        ...
        # pooled context for memory
        if self.use_mem:
            h_pool = H.mean(dim=1)
            t_pool = T.mean(dim=1)
            r_pool = R.mean(dim=1)
            mem_in = torch.cat([h_pool, t_pool, r_pool, W_tok.squeeze(1)], dim=-1)
            mem_in = self.mem_in(mem_in)
            if h_prev is None:
                h_prev = torch.zeros(
                    B, self.mem_window + 1, self.mem_dim,
                    device=mem_in.device, dtype=mem_in.dtype
                )
            memslot = h_prev[:, :1, :]
            window = h_prev[:, 1:, :]
            mem_tokens = torch.cat([memslot, window, mem_in.unsqueeze(1)], dim=1)
            mem_tokens = mem_tokens + self.mem_pos.unsqueeze(0)
            mem_tokens = self.mem_cell(mem_tokens)
            mem_state = mem_tokens[:, 0, :]
            mem_tok = mem_state.unsqueeze(1)
            if self.mem_window > 0:
                if window.shape[1] > 1:
                    window_next = torch.cat([window[:, 1:, :], mem_in.unsqueeze(1)], dim=1)
                else:
                    window_next = mem_in.unsqueeze(1)
            else:
                window_next = window
            mem_next = torch.cat([mem_state.unsqueeze(1), window_next], dim=1)
        else:
            mem_state = None
            mem_tok = None
            mem_next = None

        ctx_tokens = [H, T_pool, R_pool, W_tok]
        if mem_tok is not None:
            ctx_tokens.append(mem_tok)
        ctx_pool = torch.cat(ctx_tokens, dim=1)  # (B, V + K + D + 1 (+1), d)
        ctx_pool = torch.nan_to_num(ctx_pool, nan=0.0, posinf=1e6, neginf=-1e6)
        ctx_pool = ctx_pool.clamp(min=-20.0, max=20.0)
        ctx_pool = self.ctx_mix(ctx_pool)                   # (B,*,d)
        ...
        if self.use_gate:
            ...
            if mem_state is None:
                mem_state = torch.zeros_like(t_sum)
            gate_in = torch.cat([t_sum, r_sum, W_tok.squeeze(1), mem_state], dim=-1)
        ...
        if mem_state is None:
            mem_state = torch.zeros_like(global_feat)
        critic_in = torch.cat([global_feat, W_tok.squeeze(1), mem_state], dim=-1)
        V1 = self.critic1(critic_in)
        V2 = self.critic2(critic_in)
        w = _normalize_weights(w_pair)
        value = w[ : , :1] * V1 + w[ : , 1:2] * V2
    ...
    def init_hidden(self, batch_size: int, device: Optional[torch.device] = None):
        ...
        if self.use_mem:
            return torch.zeros(batch_size, self.mem_window + 1, self.mem_dim, device=dev)
        return torch.zeros(batch_size, self.mem_dim, device=dev)
    ...
    @torch.no_grad()
    def act(self, obs: dict, h_prev: Optional[torch.Tensor] = None, deterministic: bool = False):
        """
        If deterministic=False  -> sample stochastically (for PPO training).
        If deterministic=True   -> pick argmax actions (for clean eval).

        Returns: (action_tuple, logp, value, extra_out)
                 action_tuple = (veh_type, veh_idx, node_j)
        """
        out = self.forward(
            obs["graph_ctx"], obs["trucks_ctx"], obs["drones_ctx"],
            obs["mask_trk"],  obs["mask_dr"],  obs["weights"],
        ...
        has_trk_moves = bool(inst_has_node_trk.any().item())
        has_dr_moves  = bool(inst_has_node_dr.any().item())
        ...
        if deterministic:
            ...
            if gate_logits is not None:
                gl = torch.nan_to_num(gate_logits, nan=0.0, posinf=0.0, neginf=0.0)
                chosen = int(gl.argmax(dim=-1).item())  # 0=truck, 1=drone
            else:
                # simple deterministic fallback: prefer branch with more feasible mass
                if has_trk_moves and not has_dr_moves:
                    chosen = 0
                elif has_dr_moves and not has_trk_moves:
                    chosen = 1
                elif has_trk_moves and has_dr_moves:
                    mass_trk = mask_trk[0].float().sum().item()
                    mass_dr  = mask_dr[0].float().sum().item()
                    chosen = 0 if mass_trk >= mass_dr else 1
                else:
                    # no moves at all
                    return (0, 0, 0), torch.zeros(1, 1, device=device), out["value"], out, h_next

            # override dead branch if needed
            if chosen == 0 and (not has_trk_moves) and has_dr_moves:
                chosen = 1
            elif chosen == 1 and (not has_dr_moves) and has_trk_moves:
                chosen = 0
            elif (not has_trk_moves) and (not has_dr_moves):
                return (0, 0, 0), torch.zeros(1, 1, device=device), out["value"], out

            def best_flat_from_logp(logp_2d: torch.Tensor, mask_2d: torch.Tensor):
                """
                logp_2d: (I,V) log-probs (can contain -inf for infeasible)
                mask_2d: (I,V) booleans (True = feasible)
                Returns (inst_idx, node_idx) by argmax over feasible entries.
                """
                I, V = logp_2d.shape
                flat_logp = logp_2d.view(-1)
                ...
                safe = torch.where(
                    flat_mask,
                    flat_logp,
                    torch.full_like(flat_logp, -float("inf")),
                )
                if torch.isneginf(safe).all():
                    return None

                idx = int(safe.argmax().item())
                inst = idx // V
                node = idx % V
                return inst, node

            if chosen == 0:
                # TRUCK: greedy over (k,j)
                trk_logp = out["trk_logp"][0]          # (K,V)
                res = best_flat_from_logp(trk_logp, mask_trk[0])
                if res is None:
                    # nothing feasible -> noop
                    return (0, 0, 0), torch.zeros(1, 1, device=device), out["value"], out, h_next
                k, j = res
                # we don't care about logp in eval -> just return 0
                return (0, k, j), torch.zeros(1, 1, device=device), out["value"], out, h_next
            else:
                # DRONE: greedy over (d,j)
                dr_logp = out["dr_logp"][0]            # (D,V)
                res = best_flat_from_logp(dr_logp, mask_dr[0])
                if res is None:
                    # try trucks as last resort
                    if has_trk_moves:
                        trk_logp = out["trk_logp"][0]
                        res2 = best_flat_from_logp(trk_logp, mask_trk[0])
                        if res2 is None:
                            return (0, 0, 0), torch.zeros(1, 1, device=device), out["value"], out, h_next
                        k, j = res2
                        return (0, k, j), torch.zeros(1, 1, device=device), out["value"], out, h_next
                        ...
                        return (0, 0, 0), torch.zeros(1, 1, device=device), out["value"], out, h_next
                d, j = res
                return (1, d, j), torch.zeros(1, 1, device=device), out["value"], out, h_next

        # --------- stochastic mode (existing behaviour) ----------
        # (unchanged from your current code)
        if gate_logits is None:
            veh_dist = torch.distributions.Categorical(
        ...
        # override when sampled a dead branch
        if chosen == 1 and (not has_dr_moves) and has_trk_moves:
            chosen = 0
            veh_logp_sel = veh_dist.log_prob(torch.tensor([0], device=device)).unsqueeze(-1)
        elif chosen == 0 and (not has_trk_moves) and has_dr_moves:
            chosen = 1
            veh_logp_sel = veh_dist.log_prob(torch.tensor([1], device=device)).unsqueeze(-1)
        # elif both dead → keep as is, env should be done

        def sample_flat_from_logp(logp_2d, mask_2d):
            row_has = mask_2d.any(dim=-1)
            if not row_has.any():
                return None
            I, V = logp_2d.shape
            flat_logits = logp_2d.view(-1)
            flat_mask   = mask_2d.view(-1)
            if not flat_mask.any():
                return None
            safe_logits = torch.where(
                flat_mask,
                flat_logits,
                torch.tensor(-float("inf"), device=flat_logits.device),
            )
            if torch.isneginf(safe_logits).all():
                return None
            dist = torch.distributions.Categorical(logits=safe_logits)
        ...
        if chosen == 0:
            ...
            if ok is None:
                ...
                k, j, lp = ok
                total_logp = lp.unsqueeze(0).unsqueeze(0) + veh_logp_sel
                return (0, k, j), total_logp, out["value"], out, h_next
        ...
        if ok is None:
            ...
            d, j, lp = ok
            total_logp = lp.unsqueeze(0).unsqueeze(0) + veh_logp_sel
            return (1, d, j), total_logp, out["value"], out, h_next



    @torch.no_grad()
    def value_only(self, obs: dict, h_prev: Optional[torch.Tensor] = None):
        out = self.forward(
            obs["graph_ctx"], obs["trucks_ctx"], obs["drones_ctx"],
            obs["mask_trk"],  obs["mask_dr"],  obs["weights"],
            h_prev=h_prev,
    ...
    def evaluate_actions(self, flat_obs: dict, actions: torch.Tensor, h_prev: Optional[torch.Tensor] = None):
        """
